...
uint32_t internal_node_find_child(void *node, uint32_t key)
{
  ...
  while (min_index != max_index)
  {
    ...
    uint32_t key_to_right = *(uint32_t *)internal_node_key(node, index);
    if (key_to_right >= key)
    {
      max_index = index;
    }
...
void pager_flush(Pager *pager, uint32_t page_num)
{
  ...
  __ssize_t bytes_written = write(pager->file_descriptor, pager->pages[page_num], PAGE_SIZE);
...
uint32_t get_node_max_key(void *node)
{
  ...
  {
  case NODE_INTERNAL:
    return *internal_node_key(node, *(uint32_t *)internal_node_num_keys(node) - 1);
  case NODE_LEAF:
    return *(uint32_t *)leaf_node_key(node, *(uint32_t *)leaf_node_num_cells(node) - 1);
  }
  return 0;
}
...
void create_new_root(Table *table, uint32_t right_child_page_num)
{
  ...
  *node_parent(root) = table->root_page_num;
}
